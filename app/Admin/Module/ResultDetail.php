...
class ResultDetail extends Card
{
    ...
    public function handle(Request $request)
    {
        ...
        $this->marketing = Grid::make($m_build, function (Grid $grid) use ($id) {
            ...
            $grid->model()->where("activity_id", $id)->where("open_status", 1)->where("chip_num_all", ">", 0);
            ...
            $grid->column('child', '购买详情')->display(function ($childs) {
                $str = "";
                foreach ($childs as $child) {
                    if($child->type == 1) {
                        ...
                        $str .= "E玩法:" .$child->chip_num. "X" . $child->number . "\n";
                    }
                }
                return $str;
            })->width("30%");
